			...
			if len(p.RFC2136.Nameserver) == 0 {
				...
				for _, ns := range p.RFC2136.Nameservers {
					if _, err := util.ValidNameserver(ns); err != nil {
						el = append(el, field.Invalid(fldPath.Child("rfc2136", "nameservers"), ns, "nameserver must be set in the form host:port where host is an IPv4 address, an enclosed IPv6 address or a hostname and port is an optional port number."))
					}
				}
			}
